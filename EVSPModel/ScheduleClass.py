...
class Schedule():
    """
    A schedule is an objective which contains a series of vehicles and their service trips and
    charging activities scheduled according to a given timetable.

    A schedule has these attributes:
        schedule: including several duty, list
        cost: including vehicle, charging and labor cost
        energy feasibility: True if meet the energy constraint, else False
        capacity feasibility: True if meet the energy constraint, else False
    """

    __slots__ = ['schedule','R','vehicleCost','laborCost','chargingCost','totalCost','evsp']

    def __init__(self, evsp:EVSP, schedule=[], R={}):
        """
        schedule: initial schedule, list[Duty]
        """
    ...
    def sortDuty(self):
        ...
        """
        newS = []
        startT = {}  # duty : start-time-of-first-trip
        for duty in self.schedule:
    ...
    def checkEnergyFeasibility(self):
        """
        Return True if a schedule meet the energy constraint, else False
        """
        for duty in self.schedule:
    ...
    def checkCapacityFeasibility(self):
        """
        Return True if a duty can meet the capacity constraint, else Flase.
        """
        feasibility = True
    ...
    def calCost(self):
        """
        Calculate schedule cost including vehicle, labor and charging cost.
        """
        self.totalCost = 0
        self.vehicleCost = 0
        self.laborCost = 0
    ...
    def printTable(self):
        """
        Print out a table(DataFrame) of schedule.
        """
        self.sortDuty()
    ...
    #     ax.set_ylabel("车辆编号（车型）", fontsize=25)
    #     # ax.set_ylabel("Bus  Number", fontsize=20)

    #     # plt.title("EBs Schedule", fontsize=20)
    #     legend_elements = [
    #         Patch(facecolor='silver', label='服务行程'),
    #         Patch(facecolor='lightgreen', label='充电行程')
    #     ]
    ...
    # def costBar(self):
    #     """
    #     Display cost composition of a schedule.
    #     """
    #     _cost = self.calCost()
    #     fig, ax = plt.subplots(1,1,figsize=(6,6))
    #     costs = [round(self.vehicleCost,1), round(self.chargingCost,1), round(self.laborCost,1)]
    #     labels = ['Purchase Cost', 'Electricity Cost', 'Labor Cost']
